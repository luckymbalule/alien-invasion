...
## Description
In Alien Invasion, a player controls a rocket ship that is anchored to the bottom of the screen. The player can move the rocket ship left or right using arrow keys and shoot using [Z] key.
...
## Build Executable
To package the game into a standalone executable (no Python installation required):
```bash
pip install pyinstaller
pyinstaller --noconsole alien_invasion.py
```
Note: You must manually copy the images/ directory into the resulting dist/alien_invasion/ folder before running the executable.